...
public class UserController {
    ...
    // 회원가입 - 로그인X
    @PostMapping("/join")
    public String join(User user) { // (User user) 넣으면 username, password, email 채워진다. -> createDate는 안채워진다 -> 날렸음
        // -> form 의 main이 굉장히 중요!!!
    ...
    // 로그인 - - 로그인X
    @PostMapping("/login")
    public String login(HttpServletRequest request, User user) { // user 받음
        HttpSession session = request.getSession(); // 쿠키의 sessionId: 85를 들고옴
        ...
        // 1. db연결해서 username, password있는 지 확인
        // 2. 있으면 session영역에 인증됨 이라고 메세지 하나 넣어두자.
